...
{
    private static readonly StringName IdleAnimationName =
        new("idle");
    private static readonly StringName TravelAnimationName =
        new("travel");
    private static readonly StringName UnarmedAttackAnimationName =
        new("attack_unarmed");
    private static readonly StringName IncapacitatedAnimationName =
        new("incapacitated");
    ...
    public HeroProgressionState Progression { get; } = new();
    public bool IsGodMode =>
        Health.IsLocked && Resource.IsLocked;

    /// <summary>
    /// Toggles the debug god-mode locks for health and the hero's active class
    /// resource. Enabling restores both pools before locking them.
    /// </summary>
    public void SetGodMode(bool enabled)
    {
        Health.SetLocked(enabled);
        Resource.SetLocked(enabled);
    }
        ...
        IReadOnlyList<AbilityDefinition>? abilities = null)
    {
        ...
        Progression.Changed -= OnProgressionStatsChanged;
    ...
    /// <summary>
    /// Parent for the base body and every future equipment overlay. Movement
    /// bobbing and attack lunges affect this node without moving the HUD.
    /// </summary>
    [Export]
    public Node2D CharacterArtRoot { get; set; } = null!;

    /// <summary>
    /// Base warrior animation layer. Its SpriteFrames resource must contain
    /// idle, travel, attack_unarmed, and incapacitated animations.
    /// </summary>
    [Export]
    public AnimatedSprite2D BaseBodySprite { get; set; } = null!;
    ...
    public override void _Ready()
    {
        ...
        _visualRestPosition = CharacterArtRoot.Position;
        ...
        SnapToFormation();
        UpdateBodyFacing();
        SyncBodyAnimationToState(forceRestart: true);
    ...
    {
        _animationTime = 0.0;
        CharacterArtRoot.Position = _visualRestPosition;
    }

    /// <summary>
    /// Selects the body animation that represents the hero's current gameplay
    /// state without restarting the same animation every rendered frame.
    /// </summary>
    private void SyncBodyAnimationToState(bool forceRestart = false)
    {
        StringName desiredAnimation = _state switch
        {
            HeroState.InFormation
                when JourneyState.CurrentState
                    == JourneyStateService.JourneyState.Traveling
                => TravelAnimationName,
            HeroState.ApproachingTarget
                => TravelAnimationName,
            HeroState.ReturningToFormation
                => TravelAnimationName,
            HeroState.Attacking
                => UnarmedAttackAnimationName,
            HeroState.Incapacitated
                => IncapacitatedAnimationName,
            _ => IdleAnimationName
        };

        float speedScale = desiredAnimation
            == UnarmedAttackAnimationName
                ? CalculateAttackAnimationSpeedScale()
                : 1.0f;
    ...
    /// <summary>
    /// Plays a named body animation when it changes. Forced playback is used
    /// when a new attack or incapacitation sequence must start at frame zero.
    /// </summary>
    ...
    {
        if (!GodotObject.IsInstanceValid(BaseBodySprite)
            || BaseBodySprite.SpriteFrames is null
            || !BaseBodySprite.SpriteFrames.HasAnimation(
                animationName))
        {
            return;
        }

        BaseBodySprite.SpeedScale =
            Mathf.Max(speedScale, 0.01f);

        bool animationChanged =
            BaseBodySprite.Animation != animationName;

        if (forceRestart)
        {
            BaseBodySprite.Stop();
            BaseBodySprite.Play(animationName);
            BaseBodySprite.SetFrameAndProgress(0, 0.0f);
            return;
        }
        ...
        bool loopingAnimation =
            ...
            || animationName == TravelAnimationName;

        if (loopingAnimation
            && !BaseBodySprite.IsPlaying())
        {
            BaseBodySprite.Play(animationName);
        }
    }
    ...
    /// <summary>
    /// Matches the six-frame attack clip to the currently authored combat
    /// attack duration so its visual impact stays aligned with damage release.
    /// </summary>
    private float CalculateAttackAnimationSpeedScale()
    {
        if (BaseBodySprite.SpriteFrames is null
            || !BaseBodySprite.SpriteFrames.HasAnimation(
                UnarmedAttackAnimationName))
        {
            return 1.0f;
        ...
        int frameCount =
            BaseBodySprite.SpriteFrames.GetFrameCount(
                UnarmedAttackAnimationName);

        float framesPerSecond =
            (float)BaseBodySprite.SpriteFrames.GetAnimationSpeed(
                UnarmedAttackAnimationName);

        float relativeFrameDuration = 0.0f;

        for (int frameIndex = 0;
            frameIndex < frameCount;
            frameIndex++)
        {
            relativeFrameDuration +=
                BaseBodySprite.SpriteFrames.GetFrameDuration(
                    UnarmedAttackAnimationName,
                    frameIndex);
        }
        ...
        float attackDuration =
            Mathf.Max(CombatProfile.AttackDuration, 0.01f);

        return Mathf.Clamp(
            clipDuration / attackDuration,
            0.05f,
            20.0f);
    }

    /// <summary>
    /// The warrior art faces left in its source frames, so FlipH mirrors every
    /// body animation whenever combat targeting changes his facing to right.
    /// </summary>
    ...
    /// <summary>
    /// Chooses facing from the hero's active movement or combat destination.
    /// Formation movement takes priority so a hero visibly turns around before
    /// walking home after combat. Normal travel restores the authored leftward
    /// facing once the hero reaches formation.
    /// </summary>
    private void UpdateFacing()
    {
        if (_state == HeroState.ReturningToFormation)
        {
            UpdateFacingTowardX(FormationPosition.X);
            return;
        }

        if (Targeting.IsValidMonsterTarget(CurrentTarget))
        {
            UpdateFacingTowardX(CurrentTarget!.GlobalPosition.X);
            return;
        }

        if (_state == HeroState.InFormation
            && JourneyState.CurrentState
                == JourneyStateService.JourneyState.Traveling)
        {
    ...
    /// <summary>
    /// Turns the hero toward a horizontal world-space destination while
    /// preserving the current facing inside the configured dead zone.
    /// </summary>
    private void UpdateFacingTowardX(float destinationX)
    {
        float horizontalDifference =
            destinationX - GlobalPosition.X;
    ...
    /// <summary>
    /// Starts the normal post-swing cooldown even when a lethal attack changes
    /// targets synchronously before the attack animation reaches EndAttack.
    /// </summary>
    ...
    private bool ValidateReferences()
    {
        ...
        valid &= Require(CharacterArtRoot, nameof(CharacterArtRoot));
        valid &= Require(BaseBodySprite, nameof(BaseBodySprite));
        ...
        float? commitmentSeconds = null)
    {
        ...
        // Only the first target acquired with no active swing recovery receives
        // the encounter-opening instant attack. Retargeting after a kill or an
        // AI target switch must preserve the current melee cooldown.
